...
export default function Researches() {
  ...
  return <>
    <SEO
      title="ASE Research Institutes"
      description="ASE Research Institutes"
    ...
    <section className='lg:pt-20 lg:px-16 pt-8 px-5 space-y-8'>
      ...
      <div className=''>
          <h2 className='font-bold text-xl lg:text-3xl text-center' style={{ fontFamily: 'Playfair Display' }}>Our Research Institutes</h2>
      </div>

    </section>

    {research_institutes.map(({acronym, title, description, image}, index) => (
      <section key={index} className='lg:py-20 lg:px-16 py-12 px-5 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-14 items-center'>
        ...
        <div className='lg:col-span-6 relative space-y-5 lg:space-y-14'>
          <h1 className='text-xl lg:text-3xl font-bold' style={{ fontFamily: 'Playfair Display' }}>{title}</h1>
          <p className='text-base' style={{ fontFamily: 'Inter' }}>
            {description}
          </p>
          <Link href={`/research/${acronym.toLowerCase()}`} className='flex'>
